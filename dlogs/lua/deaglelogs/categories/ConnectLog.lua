...
local CATEGORY = {}
CATEGORY.Name = "Connection Logs"
CATEGORY.RawName = "Connect"
CATEGORY.Order = 9
CATEGORY.Data = {}
--CATEGORY.Disabled = 1
CATEGORY.Page = 1;
CATEGORY.MaxPages = 1;
CATEGORY.Query = "time varchar(255), method varchar(255), steam_id varchar(255), steam_name varchar(255), name varchar(255), ip_address varchar(255)"
CATEGORY.SearchVars = {
...
["Name"] = {val="name",ex="[Example] Nero Silverwing"}
}
...
function CATEGORY.OpenRightClick(line,data)
	...
	local connecter = RightClick:AddSubMenu(data.name);

	connecter:AddOption("Steam Name: "..data.steam_name,function()
		SetClipboardText(data.steam_name)
		copied("\""..data.steam_name.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	connecter:AddOption("SteamID: "..data.steam_id,function()
	...
	if dLogs.canView(LocalPlayer(),"IP") then
		connecter:AddOption("IP Address: "..data.ip_address,function()
	...
	for k,v in pairs(CATEGORY.Data) do
		...
		if dLogs.version != "JB" then
			if v.method == "Connected" then
				log = Format([[[%s]%s(%s) connected to the server.]],v.method,v.name,v.role)
			else
				log = Format([[[%s]%s(%s) disconnected from the server.]],v.method,v.name,v.role)
			end
		else
			if v.method == "Connected" then
				log = Format([[[%s]%s connected to the server.]],v.method,v.name)
			else
				log = Format([[[%s]%s disconnected from the server.]],v.method,v.name)
			end
		end
