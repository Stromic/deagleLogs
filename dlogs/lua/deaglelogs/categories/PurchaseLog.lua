...
local CATEGORY = {}
CATEGORY.Name = "Purchase Logs"
CATEGORY.RawName = "Purchase"
CATEGORY.Order = 15
CATEGORY.Data = {}
CATEGORY.Pages =1;
CATEGORY.MaxPages=1;
--CATEGORY.Disabled = 1
CATEGORY.Query = "time varchar(255), steam_id varchar(255), steam_name varchar(255), name varchar(255), msg varchar(255), ip_address varchar(255), isteam varchar(255)"
CATEGORY.SearchVars = {
...
function CATEGORY.OpenRightClick(line,data)
	...
	local purchaser = RightClick:AddSubMenu(data.name)

	purchaser:AddOption("Steam Name: "..data.steam_name,function()
		SetClipboardText(data.steam_name)
		copied("\""..data.steam_name.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	purchaser:AddOption("SteamID: "..data.steam_id,function()
	...
	if dLogs.canView(LocalPlayer(),"IP") then
		purchaser:AddOption("IP Address: "..data.ip_address,function()
...
function CATEGORY.FillData()
	...
	for k,v in pairs(CATEGORY.Data) do
		if v.shipment then v.item_name = v.item_name.." shipment" end
		local log = Format([[%s(%s) purchased a %s for $%s]],v.name,v.role,v.item_name,v.price)
