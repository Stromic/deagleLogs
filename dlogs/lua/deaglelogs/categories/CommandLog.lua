...
local CATEGORY = {}
CATEGORY.Name = "Command Logs"
CATEGORY.RawName = "Command"
CATEGORY.Order = 14
CATEGORY.Data = {}
--CATEGORY.Disabled = 1
CATEGORY.Page = 1;
CATEGORY.MaxPages = 1;
CATEGORY.Query = "time varchar(255), steam_id varchar(255), steam_name varchar(255), name varchar(255), cmd varchar(255), ip_address varchar(255), args varchar(255)"
CATEGORY.SearchVars = {
["Steam ID"] = {val="steam_id",ex="[Example] STEAM_0:1:32634764",default=true},
["Name"] = {val="name",ex="[Example] Neo Bowden"},
["Tool Name"] = {val="tool",ex="[Example] dynamite"},
...
function CATEGORY.OpenRightClick(line,data)
	...
	local commander = RightClick:AddSubMenu(data.name);

	commander:AddOption("Steam Name: "..data.steam_name,function()
		SetClipboardText(data.steam_name)
		copied("\""..data.steam_name.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	commander:AddOption("SteamID: "..data.steam_id,function()
	...
	if dLogs.canView(LocalPlayer(),"IP") then
		commander:AddOption("IP Address: "..data.ip_address,function()
...
function CATEGORY.FillData()
	...
	for k,v in pairs(CATEGORY.Data) do
		...
		if dLogs.version != "JB" then
			log = Format([[%s(%s) entered "%s"]],v.name,v.role,v.cmd.." "..table.concat(util.JSONToTable(v.args)," "))
		else
			log = Format([[%s entered "%s"]],v.name,v.cmd.." "..table.concat(util.JSONToTable(v.args)," "))
		end
