...
local CATEGORY = {}
CATEGORY.Name = "Warrant Logs"
CATEGORY.RawName = "Warrant"
CATEGORY.Order = 8
CATEGORY.Data = {}
CATEGORY.Pages =1;
CATEGORY.MaxPages=1;
...
function CATEGORY.FillData()
	...
	for k,v in pairs(CATEGORY.Data) do
		...
		if v.method == "Warranted" then
			log = Format([[[%s]%s(%s) warranted %s(%s) for "%s"]],v.method,v.name_officer,v.orole,v.name,v.role,v.reason)
		else
			log = Format([[[%s]%s(%s)'s warrant has expired.]],v.method,v.name,v.role)
		end
