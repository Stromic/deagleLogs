...
local CATEGORY = {}
CATEGORY.Name = "Prop Spawns"
CATEGORY.RawName = "Prop"
CATEGORY.Order = 3
CATEGORY.Data = {}
CATEGORY.Pages =1;
CATEGORY.MaxPages=1;
--CATEGORY.Disabled = 1
CATEGORY.Query = "time varchar(255), steam_id varchar(255), steam_name varchar(255), name varchar(255), model varchar(255), ip_address varchar(255)"
CATEGORY.SearchVars = {
["Steam ID"] = {val="steam_id",ex="[Example] STEAM_0:1:32634764",default=true},
["Name"] = {val="name",ex="[Example] Bob Doe"},
["Prop Model"] = {val="model",ex="[Example] models/props_c17/Lockers001a.mdl"},
...
function CATEGORY.OpenRightClick(line,data)
	...
	RightClick:AddOption("Copy Model",function()
		SetClipboardText(data.model)
		copied("\""..data.model.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	local propspawner =RightClick:AddSubMenu(data.name)

	propspawner:AddOption("Steam Name: "..data.steam_name,function()
	...
	propspawner:AddOption("SteamID: "..data.steam_id,function()
	...
	if dLogs.canView(LocalPlayer(),"IP") then
		propspawner:AddOption("IP Address: "..data.ip_address,function()
...
function CATEGORY.FillData()
	...
	for k,v in pairs(CATEGORY.Data) do
		local log = Format([[%s(%s) spawned a "%s"]],v.name,v.role,v.model)
