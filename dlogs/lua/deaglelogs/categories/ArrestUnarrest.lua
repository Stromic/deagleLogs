...
local CATEGORY = {}
CATEGORY.Name = "Arrests/Unarrests"
CATEGORY.RawName = "Arrest"
CATEGORY.Order = 6
CATEGORY.Page = 1;
CATEGORY.MaxPages = 1;
CATEGORY.Data = {}
--CATEGORY.Disabled = 1
CATEGORY.Query = "time varchar(255), method varchar(255), steam_id varchar(255), steam_name varchar(255), name varchar(255),name_officer varchar(255), steam_id_officer varchar(255), steam_name_officer varchar(255), ip_address varchar(255), ip_address_officer varchar(255), arrest_time varchar(255)"
CATEGORY.SearchVars = {
["Criminal ID"] = {val="steam_id",ex="[Example] STEAM_0:1:32634764",default=true},
["Criminal Name"] = {val="name",ex="[Example] Nero Silverwing"},
["Criminal IP"] = {val="ip_address",ex="[Example] 192.1.1.254"},
["Officer ID"] = {val="steam_id_officer",ex="[Example] STEAM_0:1:32634764 "},
["Officer Name"] = {val="name_officer",ex="[Example] Bob Doe"},
["Officer IP"] = {val="ip_address_officer",ex="[Example] 192.1.1.254"},
["Time"] = {val="arrest_time",ex="[Example] 300"}
}
...
function CATEGORY.OpenRightClick(line,data)
	...
	local officer = RightClick:AddSubMenu(data.name_officer.."(Officer)")


	officer:AddOption("Steam Name: "..data.steam_name_officer,function()
		SetClipboardText(data.steam_name_officer)
		copied("\""..data.steam_name_officer.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	officer:AddOption("SteamID: "..data.steam_id_officer,function()
		SetClipboardText(data.steam_id_officer)
		copied("\""..data.steam_id_officer.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	local criminal = RightClick:AddSubMenu(data.name.."(Criminal)")

	criminal:AddOption("Steam Name: "..data.steam_name,function()
		SetClipboardText(data.steam_name)
		copied("\""..data.steam_name.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	criminal:AddOption("SteamID: "..data.steam_id,function()
	...
	if dLogs.canView(LocalPlayer(),"IP") then
		...
		officer:AddOption("IP Address: "..data.ip_address_officer,function()
			SetClipboardText(data.ip_address_officer)
			copied("\""..data.ip_address_officer.."\" copied to clipboard.")
			RightClick:Hide()
		end)

		criminal:AddOption("IP Address: "..data.ip_address,function()
	...
	if data.method == "Arrested" then

		local weps = criminal:AddSubMenu(data.name.."(Criminal) Weapons ")
			for k,v in pairs(data.wep) do

				weps:AddOption(v,function()
					SetClipboardText(v)
					copied("\""..v.."\" copied to clipboard.")
					RightClick:Hide()
				end)
			end
	end
...
function CATEGORY.FillData()
	...
	for k,v in pairs(CATEGORY.Data) do
		...
		if v.method == "Arrested" then
			log = Format([[[%s]%s(%s) arrested %s(%s) for %s seconds]],v.method,v.name_officer,v.orole,v.name,v.role,v.arrest_time)
		else
			if v.name_officer != "" then
				log = Format([[[%s]%s(%s) unarrested %s(%s)]],v.method,v.name_officer,v.orole,v.name,v.role)
			else
				log = Format([[[%s]%s(%s) was let out of jail.]],v.method,v.name,v.steam_name)
			end
		end
...
function CATEGORY.HighlightData(Name)
	...
	for k,v in pairs(lines) do
		...
		if tab.name == Name or tab.name_officer == Name then
...
if !IsValid(CATEGORY.minimalSearch) then return end
	...
	for k,v in pairs(CATEGORY.Data) do
		...
		if v.name_officer then
			CATEGORY.InvolvedPlayers[v.name_officer] = true
		end
	end
	local sortedtab = {}
