...
local CATEGORY = {}
CATEGORY.Name = "Demote Logging"
CATEGORY.RawName = "Demote"
CATEGORY.Order = 10
CATEGORY.Data = {}
CATEGORY.Page = 1;
CATEGORY.MaxPages = 1;
--CATEGORY.Disabled = 1
CATEGORY.Query = "time varchar(255), steam_id varchar(255), steam_name varchar(255), name varchar(255),name_demoter varchar(255), steam_id_demoter varchar(255), steam_name_demoter varchar(255), ip_address varchar(255), ip_address_demoter varchar(255), job varchar(255), reason varchar(255)"
CATEGORY.SearchVars = {
["Demotee ID"] = {val="steam_id",ex="[Example] STEAM_0:1:32634764",default=true},
["Demotee Name"] = {val="name",ex="[Example] Nero Silverwing"},
["Demotee IP"] = {val="ip_address",ex="[Example] 192.1.1.254"},
["Demoter ID"] = {val="steam_id_demoter",ex="[Example] STEAM_0:1:32634764 "},
["Demoter Name"] = {val="name_demoter",ex="[Example] Bob Doe"},
["Demoter IP"] = {val="ip_address_demoter",ex="[Example] 192.1.1.254"},
["Reason"] = {val="reason",ex="[Example] RDM"},
["Job"] = {val="job",ex="[Example] Hitman"}
}
...
function CATEGORY.OpenRightClick(line,data)
	...
	local demoter = RightClick:AddSubMenu(data.name_demoter.."(Demoter)")

	demoter:AddOption("Steam Name: "..data.steam_name_demoter,function()
		SetClipboardText(data.steam_name_demoter)
		copied("\""..data.steam_name_demoter.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	demoter:AddOption("SteamID: "..data.steam_id_demoter,function()
		SetClipboardText(data.steam_id_demoter)
		copied("\""..data.steam_id_demoter.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	local victim = RightClick:AddSubMenu(data.name.."(Demotee)")
	...
	if dLogs.canView(LocalPlayer(),"IP") then
		...
		demoter:AddOption("IP Address: "..data.ip_address_demoter,function()
			SetClipboardText(data.ip_address_demoter)
			copied("\""..data.ip_address_demoter.."\" copied to clipboard.")
			RightClick:Hide()
		end)
...
function CATEGORY.FillData()
	...
	for k,v in pairs(CATEGORY.Data) do
		local log = Format([[%s(%s) demoted %s(%s) for "%s"]],v.name_demoter,v.drole,v.name,v.role,v.reason)
...
function CATEGORY.HighlightData(Name)
	...
	for k,v in pairs(lines) do
		...
		if tab.name == Name or tab.name_demoter == Name then
...
if !IsValid(CATEGORY.minimalSearch) then return end
	...
	for k,v in pairs(CATEGORY.Data) do
		...
		if v.name_demoter then
			CATEGORY.InvolvedPlayers[v.name_demoter] = true
		end
	end
	local sortedtab = {}
