...
local CATEGORY = {}
CATEGORY.Name = "Hit Logs"
CATEGORY.RawName = "Hit"
CATEGORY.Order = 11
CATEGORY.Data = {}
CATEGORY.Pages =1;
CATEGORY.MaxPages=1;
--CATEGORY.Disabled = 1
CATEGORY.Query = "time varchar(255), method varchar(255), steam_id varchar(255), steam_name varchar(255), name varchar(255),name_hitman varchar(255), steam_id_hitman varchar(255), steam_name_hitman varchar(255), ip_address varchar(255), ip_address_hitman varchar(255), ip_address_customer varchar(255), reason varchar(255), steam_id_customer varchar(255), steam_name_customer varchar(255), name_customer varchar(255)"
CATEGORY.SearchVars = {
["Victim ID"] = {val="steam_id",ex="[Example] STEAM_0:1:32634764",default=true},
["Victim Name"] = {val="name",ex="[Example] Nero Silverwing"},
["Victim IP"] = {val="ip_address",ex="[Example] 192.1.1.254"},
["Hitman ID"] = {val="steam_id_hitman",ex="[Example] STEAM_0:1:32634764 "},
["Hitman Name"] = {val="name_hitman",ex="[Example] Bob Doe"},
["Hitman IP"] = {val="ip_address_hitman",ex="[Example] 192.1.1.254"},
["Customer ID"] = {val="steam_id_customer",ex="[Example] STEAM_0:1:32634764 "},
["Customer Name"] = {val="name_customer",ex="[Example] Bob Doe"},
["Customer IP"] = {val="ip_address_customer",ex="[Example] 192.1.1.254"},
}
...
function CATEGORY.OpenRightClick(line,data)
	...
	local hitman = RightClick:AddSubMenu(data.name_hitman.."(Hitman)")

	hitman:AddOption("Steam Name: "..data.steam_name_hitman,function()
		SetClipboardText(data.steam_name_hitman)
		copied("\""..data.steam_name_hitman.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	hitman:AddOption("SteamID: "..data.steam_id_hitman,function()
		SetClipboardText(data.steam_id_hitman)
		copied("\""..data.steam_id_hitman.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	local victim = RightClick:AddSubMenu(data.name.."(Victim)")


	victim:AddOption("Steam Name: "..data.steam_name,function()
		SetClipboardText(data.steam_name)
		copied("\""..data.steam_name.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	victim:AddOption("SteamID: "..data.steam_id,function()
		SetClipboardText(data.steam_id)
		copied("\""..data.steam_id.."\" copied to clipboard.")
		RightClick:Hide()
	end)
	local customer;
	if data.name_customer then
		customer = RightClick:AddSubMenu(data.name_customer.."(Customer)")

		customer:AddOption("Steam Name: "..data.steam_name_customer,function()
			SetClipboardText(data.steam_name_customer)
			copied("\""..data.steam_name_customer.."\" copied to clipboard.")
			RightClick:Hide()
		end)

		customer:AddOption("SteamID: "..data.steam_id_customer,function()
			SetClipboardText(data.steam_id_customer)
			copied("\""..data.steam_id_customer.."\" copied to clipboard.")
			RightClick:Hide()
		end)
	end

	if dLogs.canView(LocalPlayer(),"IP") then
		hitman:AddOption("IP Address: "..data.ip_address_hitman,function()
			SetClipboardText(data.ip_address_hitman)
			copied("\""..data.ip_address_hitman.."\" copied to clipboard.")
			RightClick:Hide()
		end)

		victim:AddOption("IP Address: "..data.ip_address,function()
		...
		if data.name_customer then
			customer:AddOption("IP Address: "..data.ip_address_customer,function()
				SetClipboardText(data.ip_address_customer)
				copied("\""..data.ip_address_customer.."\" copied to clipboard.")
				RightClick:Hide()
			end)
		end
...
function CATEGORY.FillData()
	...
	if !IsValid(CATEGORY.ListView) then return end
	CATEGORY.ListView:Clear()
	for k,v in pairs(CATEGORY.Data) do
		local log
		if v.method == "Accepted" then
			log = Format([[[%s]%s(%s) ordered a hit from %s(%s) on %s(%s)]],v.method,v.name_customer,v.crole,v.name_hitman,v.hrole,v.name,v.role)
		elseif v.method == "Failed" then
			log = Format([[[%s]%s(%s) failed their hit on %s(%s) for "%s"]],v.method,v.name_hitman,v.hrole,v.name,v.role,v.reason)
		elseif v.method == "Completed" then
			log = Format([[[%s]%s(%s) completed their hit on %s(%s) ordered by %s(%s)]],v.method,v.name_hitman,v.hrole,v.name,v.role,v.name_customer,v.crole)
		end
...
function CATEGORY.HighlightData(Name)
	...
	for k,v in pairs(lines) do
		...
		if tab.name == Name or tab.name_customer == Name or tab.name_hitman == Name then
...
if !IsValid(CATEGORY.minimalSearch) then return end
	...
	for k,v in pairs(CATEGORY.Data) do
		if v.name then
			CATEGORY.InvolvedPlayers[v.name] = true
		end
		if v.name_hitman then
			CATEGORY.InvolvedPlayers[v.name_hitman] = true
		end
		if v.name_customer then
			CATEGORY.InvolvedPlayers[v.name_customer] = true
		end
	end
	local sortedtab = {}
