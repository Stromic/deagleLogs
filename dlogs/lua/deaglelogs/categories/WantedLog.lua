...
local CATEGORY = {}
CATEGORY.Name = "Wanted Logs"
CATEGORY.RawName = "Wanted"
CATEGORY.Order = 7
CATEGORY.Data = {}
CATEGORY.Pages =1;
CATEGORY.MaxPages=1;
--CATEGORY.Disabled = 1
CATEGORY.Query = "time varchar(255), method varchar(255), steam_id varchar(255), steam_name varchar(255), name varchar(255),name_officer varchar(255), steam_id_officer varchar(255), steam_name_officer varchar(255), ip_address varchar(255), ip_address_officer varchar(255), reason varchar(255)"
CATEGORY.SearchVars = {
...
["Reason"] = {val="reason",ex="[Example] Theft"}
}
...
function CATEGORY.FillData()
	...
	for k,v in pairs(CATEGORY.Data) do
		...
		if v.method == "Wanted" then
			log = Format([[[%s]%s(%s) wanted %s(%s) for "%s"]],v.method,v.name_officer,v.orole,v.name,v.role,v.reason)
		else
			if v.name_officer == "_[EX]_" then
				log = Format([[[%s]%s(%s) is no longer wanted]],v.method,v.name,v.role)
			else
				log = Format([[[%s]%s(%s) unwanted %s(%s)]],v.method,v.name_officer,v.orole,v.name,v.role)
			end
		end
