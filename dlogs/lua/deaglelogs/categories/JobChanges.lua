...
local CATEGORY = {}
CATEGORY.Name = "Job Changes"
CATEGORY.RawName = "Job"
CATEGORY.Order = 2
CATEGORY.Data = {}
CATEGORY.Pages =1;
CATEGORY.MaxPages=1;
--CATEGORY.Disabled = 1
CATEGORY.Query = "time varchar(255), steam_id varchar(255), steam_name varchar(255), name varchar(255), oldjob varchar(255), newjob varchar(255), ip_address varchar(255)"
CATEGORY.SearchVars = {
["Steam ID"] = {val="steam_id",ex="[Example] STEAM_0:1:32634764",default=true},
["Name"] = {val="name",ex="[Example] Neo Bowden"},
["Old Job"] = {val="oldjob",ex="[Example] Thief"},
["New Job"] = {val="newjob",ex="[Example] Hobo"},
...
function CATEGORY.OpenRightClick(line,data)
	...
	local jobber = RightClick:AddSubMenu(data.name)

	jobber:AddOption("Steam Name: "..data.steam_name,function()
		SetClipboardText(data.steam_name)
		copied("\""..data.steam_name.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	jobber:AddOption("SteamID: "..data.steam_id,function()
	...
	if dLogs.canView(LocalPlayer(),"IP") then
		jobber:AddOption("IP Address: "..data.ip_address,function()
...
function CATEGORY.FillData()
	...
	for k,v in pairs(CATEGORY.Data) do
		local log = Format([[%s(%s) changed their job from "%s" to "%s"]],v.name,v.steam_name,v.oldjob,v.newjob)
