...
local CATEGORY = {}
CATEGORY.Name = "Chat Logs"
CATEGORY.RawName = "Chat"
CATEGORY.Order = 13
CATEGORY.Page = 1;
CATEGORY.MaxPages = 1;
CATEGORY.Data = {}
...
function CATEGORY.OpenRightClick(line,data)
	...
	local chatter = RightClick:AddSubMenu(data.name);

	chatter:AddOption("Steam Name: "..data.steam_name,function()
		SetClipboardText(data.steam_name)
		copied("\""..data.steam_name.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	chatter:AddOption("SteamID: "..data.steam_id,function()
	...
	if dLogs.canView(LocalPlayer(),"IP") then
		chatter:AddOption("IP Address: "..data.ip_address,function()
	...
	local log
		if dLogs.version != "JB" then
			if v.isteam == "false" then
				log = Format([[%s(%s) said "%s"]],v.name,v.role,v.msg)
			else
				log = Format([[[TEAM]%s(%s) said "%s"]],v.name,v.role,v.msg)
			end
		else
			if v.isteam == "false" then
				log = Format([[%s(%s) said "%s"]],v.name,v.role,v.msg)
