...
local CATEGORY = {}
CATEGORY.Name = "Tool Logs"
CATEGORY.RawName = "Tool"
CATEGORY.Order = 4
CATEGORY.Data = {}
CATEGORY.Pages =1;
CATEGORY.MaxPages=1;
--CATEGORY.Disabled = 1
CATEGORY.Query = "time varchar(255), steam_id varchar(255), steam_name varchar(255), name varchar(255), tool varchar(255), ip_address varchar(255)"
CATEGORY.SearchVars = {
...
function CATEGORY.OpenRightClick(line,data)
	...
	local tooler = RightClick:AddSubMenu(data.name)

	tooler:AddOption("Steam Name: "..data.steam_name,function()
		SetClipboardText(data.steam_name)
		copied("\""..data.steam_name.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	tooler:AddOption("SteamID: "..data.steam_id,function()
	...
	if dLogs.canView(LocalPlayer(),"IP") then
		tooler:AddOption("IP Address: "..data.ip_address,function()
...
function CATEGORY.FillData()
	...
	for k,v in pairs(CATEGORY.Data) do
		local log = Format([[%s(%s) spawned a "%s"]],v.name,v.role,v.tool)
