...
local CATEGORY = {}
CATEGORY.Name = "Damage Logs"
CATEGORY.RawName = "Damage"
CATEGORY.Order = 12
CATEGORY.Data = {}
--CATEGORY.Disabled = 1
CATEGORY.Page = 1;
CATEGORY.MaxPages = 1;
CATEGORY.Query = "time varchar(255), method varchar(255), steam_id varchar(255), steam_name varchar(255), name varchar(255),name_killer varchar(255), steam_id_killer varchar(255), steam_name_killer varchar(255), ip_address varchar(255), ip_address_killer varchar(255), weapon varchar(255), damage varchar(255)"
CATEGORY.SearchVars = {
...
["Killer ID"] = {val="steam_id_killer",ex="[Example] STEAM_0:1:32634764 "},
["Killer Name"] = {val="name_killer",ex="[Example] Bob Doe"},
["Killer IP"] = {val="ip_address_killer",ex="[Example] 192.1.1.254"},
["Weapon"] = {val="weapon",ex="[Example] prop_physics"}
}
...
function CATEGORY.OpenRightClick(line,data)
	...
	local killer = RightClick:AddSubMenu(data.name_killer.."(Killer)")

	killer:AddOption("Steam Name: "..data.steam_name_killer,function()
		SetClipboardText(data.steam_name_killer)
		copied("\""..data.steam_name_killer.."\" copied to clipboard.")
		RightClick:Hide()
	end)

	killer:AddOption("SteamID: "..data.steam_id_killer,function()
		SetClipboardText(data.steam_id_killer)
		copied("\""..data.steam_id_killer.."\" copied to clipboard.")
		RightClick:Hide()
	end)
	...
	if dLogs.canView(LocalPlayer(),"IP") then
		...
		killer:AddOption("IP Address: "..data.ip_address_killer,function()
			SetClipboardText(data.ip_address_killer)
			copied("\""..data.ip_address_killer.."\" copied to clipboard.")
			RightClick:Hide()
		end)
...
function CATEGORY.FillData()
	...
	for k,v in pairs(CATEGORY.Data) do
		...
		if dLogs.version != "JB" then
			if v.method == "DMG" then
				log = Format([[[%s]%s(%s) hurt %s(%s) for %s damage using %s]],v.method,v.name_killer,v.role_killer,v.name,v.role,tostring(v.damage),tostring(v.weapon))
			else
				log = Format([[[%s]%s(%s) killed %s(%s) with %s damage using %s]],v.method,v.name_killer,v.role_killer,v.name,v.role,tostring(v.damage),tostring(v.weapon))
			end
		else
			if v.method == "DMG" then
				log = Format([[[%s]%s(%s) hurt %s(%s) for %s damage using %s]],v.method,v.name_killer,v.role_killer,v.name,v.role,tostring(v.damage),tostring(v.weapon))
...
function CATEGORY.HighlightData(Name)
	...
	for k,v in pairs(lines) do
		...
		if tab.name == Name or tab.name_killer == Name then
...
if !IsValid(CATEGORY.minimalSearch) then return end
	...
	for k,v in pairs(CATEGORY.Data) do
		if v.name then
			...
			if v.name_killer then
				CATEGORY.InvolvedPlayers[v.name_killer] = true
			end
	end
	local sortedtab = {}
